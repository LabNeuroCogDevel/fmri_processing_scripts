...
setup() {
 ...
 3dTcat -prefix "${prefix}$funcNifti" temp.nii.gz temp.nii.gz temp+10.nii.gz temp.nii.gz temp.nii.gz 
...
@test "take first 2" {
  n_rm_firstvols=2
      ...
      nt_pre=$(3dinfo -nt $prefix$funcNifti)
   mean0_pre=$(3dBrickStat -mean $prefix$funcNifti'[0]')
  prefix_pre=$prefix
  ...
  nt_post=$(3dinfo -nt $prefix$funcNifti)
  let "expect = nt_pre - n_rm_firstvols"
  [ $nt_post -eq $expect ]

  mean0_post=$(3dBrickStat -mean $prefix$funcNifti'[0]')
  mean1_post=$(3dBrickStat -mean $prefix$funcNifti'[1]')
